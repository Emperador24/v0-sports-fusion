...
import { ChevronRight, Save, RotateCcw, Route, Check } from "lucide-react"
...
function RegistroDistanciaContent() {
  ...
  const [ritmo, setRitmo] = useState<string>("0:00")
  ...
  // Calcular ritmo cuando cambia la distancia o el tiempo
  useEffect(() => {
    if (distancia && (horas !== "" || minutos !== "" || segundos !== "")) {
      const totalSeconds = convertToSeconds(horas, minutos, segundos)
      const distanciaNum = Number.parseFloat(distancia.toString())

      if (distanciaNum > 0 && totalSeconds > 0) {
        // Calcular ritmo en segundos por kilómetro
        const ritmoEnSegundos = Math.floor(totalSeconds / distanciaNum)

        // Convertir a formato min:seg
        const ritmoMinutos = Math.floor(ritmoEnSegundos / 60)
        const ritmoSegundos = ritmoEnSegundos % 60

        setRitmo(`${ritmoMinutos}:${ritmoSegundos.toString().padStart(2, "0")}`)
      } else {
        setRitmo("0:00")
      }
    }
  }, [distancia, horas, minutos, segundos])
              ...
              <form onSubmit={handleSubmit}>
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="distancia" className="text-white text-lg font-medium flex items-center">
                      <Route className="h-5 w-5 mr-2 text-purple-400" />
                      DISTANCIA (km)
                    </Label>
                    <div className="relative">
                      <div className="absolute -inset-1 bg-gradient-to-r from-purple-500/30 to-[#4DFF9F]/30 rounded-lg blur-sm"></div>
                      <Input
                        id="distancia"
                        type="number"
                        step="0.01"
                        ...
                        className="relative bg-gray-900/80 border-gray-700 text-white text-center text-2xl h-16 placeholder:text-gray-600 focus:border-purple-500 focus:ring-purple-500"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center">
                      <Clock className="h-6 w-6 mr-2 text-[#4D9FFF]" /> {/* Use Clock component */}
                  ...
                  <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-400">Ritmo (min/km):</span>
                      <span className="text-xl font-semibold text-white">{ritmo}</span>
                    </div>
                  </div>
          ...
          <Card className="col-span-1 bg-gradient-to-b from-gray-900 to-[#050505] border-gray-800">
            <CardContent className="p-6">
              ...
              <div className="space-y-4 text-gray-400 text-sm">
                <p>Registra la distancia recorrida y el tiempo empleado para hacer seguimiento de tu progreso.</p>

                <div className="pt-2 border-t border-gray-800">
                  <h4 className="text-white text-base mb-2">Consejos:</h4>
                  <ul className="list-disc list-inside space-y-1">
                    <li>Ingresa la distancia en kilómetros</li>
                    <li>El ritmo se calcula automáticamente</li>
                    <li>Puedes registrar actividades de cualquier duración</li>
                  </ul>
                </div>
              </div>
            </CardContent>
...
export default function RegistroDistancia() {
  return (
    <Suspense fallback={<LoadingState />}>
      <RegistroDistanciaContent />
    </Suspense>
  )
}
